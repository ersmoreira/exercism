...
export function monthRate(ratePerHour, discount) {
  let valor = dayRate(ratePerHour) * MONTH_BILLABLE_DAYS
  return Math.ceil(applyDiscount(valor, discount));
}
...
export function daysInBudget(budget, ratePerHour, discount) {
  let val = dayRate(ratePerHour);
  val = applyDiscount(val, discount)
  return Math.trunc(budget / val)
}
